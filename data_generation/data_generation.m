...
k = 5;
n = 10000;
scale = 0.3; % to control separation of the Gaussians
...
display(X) % output is the data matrix X
...
function [mu, Sigma] = randomgaussian(d, scale)
    mu = randn(d, 1);
    Y = randn(d, d);
    Sigma = scale^2*Y.'*Y; % here, you can also add the term " + const*trace(scale^2*Y.'*Y)*eye(d) "
